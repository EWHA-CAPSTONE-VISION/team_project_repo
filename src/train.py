            ...
            try:
                ...
                for i in range(0, N, batch_spots):
                    ...
                    with autocast('cuda'):
                        ...
                        # SC, ST encoder is trainable
                        sc_feat = model.sc_encoder(expr_batch)
                        st_feat = model.st_encoder(coord_batch)
                        fusion = model.fusion(img_feat, sc_feat, st_feat)
                    
                    # Move back to CPU and store (save GPU memory)
                    spot_embeds_list.append(fusion.detach().cpu())
                    
                    # Immediately free GPU memory
                    del img_batch, expr_batch, coord_batch, img_feat, sc_feat, st_feat, fusion
                    torch.cuda.empty_cache()
